...
class LatestTVSeriesCubit extends Cubit<LatestTVSeriesStates> {
  ...
  LatestTVSeriesCubit(this._tvSeriesRepository)
    : super(const LatestTVSeriesStates());
  ...
  Future<void> getLatestTVSeries({bool refresh = false}) async {
    ...
    response.fold((l) => emit(state.copyWith(errorMessage: l.message)), (r) {
      final latestTVSeries =
          refresh ? r.results : [...state.latestTVSeries, ...r.results];
      emit(
        state.copyWith(
          latestTVSeries: latestTVSeries,
          latestTVSeriesIsLoading: false,
          latestTVSeriesHasReachedMax:
              _latestTVSeriesCurrentPage >= r.totalPages,
          errorMessage: null,
        ),
      );
